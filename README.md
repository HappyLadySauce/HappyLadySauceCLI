...
| 文件 | 内容 |
|------|------|
| `<home>/logs/happyladysaucecli.log` | 全部诊断日志（INFO 级 phase trace V=1/V=2，以及 ERROR 级如工具 recovered、持久化失败等） |

使用 `make run V=1` 打开因果主链，`make run V=2` 额外打开 model call 输入、compaction 检查、工具事件与持久化细节。

日志行以 `[Info]` 或 `[Error]` 开头，统一带 `phase=` 字段，并尽量附带 `conversation_id` 与 `model_call`，便于 grep 串联一次用户交互：

| phase | 级别 | 含义 |
|-------|------|------|
| `session_open` | V=1 | CLI 启动，context session 创建 |
| `session_close` | V=1 | CLI 退出，context session 关闭 |
| `user_turn_begin` | V=1 | 收到用户输入，含 `user_prompt_len`（不含明文） |
| `model_call_end` | V=1 | 单次 model call 完成，含 token 与 `tool_calls` |
| `capability_policy` | V=1 | 策略评估结果（allow/deny/review） |
| `capability_call` | V=1 | 工具执行完成或 recovered 拒绝/错误 |
| `agent_event` | V=1（tool）/ V=2（其他） | 工具结果、错误、退出等 agent 事件 |
| `user_turn_end` | V=1 | 用户回合结束汇总 |
| `model_call_begin` | V=2 | 每次 model call 前的输入消息统计 |
...
```text
phase=session_open session_id=session_abc
phase=user_turn_begin conversation_id=conversation_abc user_turn_seq=2 user_prompt_len=24 history_messages=3
phase=capability_policy tool_name=get_weather decision=allow decision_reason=default_policy_allow status=allowed
phase=capability_call tool_name=get_weather decision=allow approval_scope=none output_bytes=156 elapsed_ms=420 status=success
phase=agent_event kind=tool_message tool_name=get_weather content_len=156
...
phase=user_turn_end conversation_id=conversation_abc model_calls=1 turn_messages=3 history_messages=5 prompt_agg=341 completion_agg=95 content=436 elapsed_ms=2820
phase=session_close session_id=session_abc
```
